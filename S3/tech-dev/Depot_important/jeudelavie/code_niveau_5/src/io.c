...
void debut_jeu(grille *g, grille *gc){
	...
	while (c != 'q') // touche 'q' pour quitter
	{
		switch (c) {
			...
			case 'o' :
			{
				resultat=test_oscillation_delai(g,gc);
				
				if(resultat.x==-1) printf("Cette grille n'est pas du tout oscillante                                                      \n");
				else if(resultat.y==0) printf("Cette grille sera totalement morte dans %d evolution                                       \n",resultat.x);
				else printf("Une colonie oscillante a ete trouve, delai : %d , periode : %d                            \n",resultat.x,resultat.y);
